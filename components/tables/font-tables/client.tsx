...
import { Font, getAllFonts } from '@/services/fontService';
...
export const FontClient = () => {
  ...
  const [totalFonts, setTotalFonts] = useState<number>(0);
  ...
  useEffect(() => {
    fetchFonts(page, pageSize, search, sortField, sortOrder);
  ...
  return (
    <div className="flex-1 space-y-4 overflow-auto">
      <div className="flex flex-wrap items-start justify-between gap-5 sm:gap-0">
        <Heading
          title={`Fontes (${totalFonts})`}
          description="Gerencie as fontes"
        />
        <div className="flex flex-wrap items-center gap-5">
          <Button
            className="items-center gap-5 text-lg dark:text-black"
            onClick={() => router.push(`/dashboard/fontes/important`)}
          >
          ...
          <Button
            ...
            onClick={() => router.push(`/dashboard/fontes/new`)}
          >
      ...
      <DataTableComponent
        ...
        data={fonts}
        searchKey="name"
        placeholderInput="Pesquise por fonte"
        pageCount={pageCount}
        pageSize={pageSize}
        page={page}
        loading={loading}
